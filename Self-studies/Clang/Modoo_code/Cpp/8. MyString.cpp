/*  구분                                       반환 대상                     메모리                                      연속 호출 결과(메서드 체이닝)                대입 연산 (=)
    class_type& member_function() (참조 반환)   현재 객체 원본(실제 메모리)     비용없음 (주소값만 전달; 매우 가볍고 빠름)      동일한 하나의 객체에 모든 수정 누적           func() = new_obj; 처럼 대입이 가능함
    class_type member_function() (값 반환)      현재 객체 복사본               비쌈 (객체 크기가 클수록 복사 비용 증가)       호출할 때마다 복사본이 생성되어 수정 증발      func() = new_obj; 처럼 쓰면 컴파일 에러
*/
#include <iostream>
...
public:
    ...
    ~MyString();
    /*  & 없이 일반 기본 타입을 반환하는 함수들 (length, capacity, find, compare, at)
        객체 자신을 변경하는 것이 아니라, 객체로부터 특정 값을 조사해서 알려주는 역할.
        문자열 객체 자체가 아니라 단순한 정수(크기, 인덱스, 비교 결과)를 넘겨주면 끝.
        int는 크기가 매우 작아 복사 비용이 없으므로 &를 붙일 이유가 전혀 없음.
    */
    int length() const;
    int capacity() const;
    /*  void를 반환하는 함수들 (reserve, print, println)
        함수가 끝난 뒤 아무것도 돌려줄 필요가 없기 때문에, void 타입 반환
    */
    void reserve(int size);
    void print() const;
    void println() const;
    /* MyString&를 반환하는 함수들 (assign, insert, erase)
       문자열의 내용을 변경(수정)한 후, "수정된 문자열"을 가지고 연속해서 작업할 수 있도록(메서드 체이닝) 설계
    */
...
MyString& MyString::insert(int loc, const MyString& str) {
    ...
    if (string_length + str.string_length > memory_capacity) { // 문자열 반복 추가 대비, 현재 메모리 크기 두배 할당.
        ...
        //for (초기화; 조건; 증감) 이므로 초기화를 하지 않으므로 i에 대한 초기화 없이 그냥 ;
        //i를 루프 내 초기화 시, 루프 바깥에서는 정의가 안되어, 다른 루프에서 다시 정의해야 됨.
        int i = 0;
        for (; i < loc; i++) { string_content[i] = prev_string_content[i]; }
...
MyString& MyString::insert(int loc, const char* str) {
    MyString temp(str);
    return insert(loc, temp); // 같은 클래스 내 wrapping할땐 그냥 method 명만.
}
...
int main() {
    ...
    str1.reserve(30);
    std::cout << str1.at(10) << str1.at(25) << str1.at(31) << std::endl;
